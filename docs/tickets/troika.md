...
Наземный транспорт, метро и на МЦД прямо на турникете или валидаторе с бесплатной пересадкой на метро и МЦК в течение 90 минут.
...
{% cut "Что будет если не валидировать Тройку?" %}{name=troikagroup}
...
{% cut "Если не будет достаточно денег на Тройке при проходе по Кошельку - что будет?" %}{name=troikagroup}
...
{% cut "Что будет если я уеду дальше зоны Дальняя и у меня только Тройка?" %}{name=troikagroup}
...
{% cut "У меня на Тройке записан пригородный билет и есть деньги на Кошельке - будет ли списание с Кошелька?" %}{name=troikagroup}
...
{% cut "Я могу записать билет Экспресс на Тройку?" %}{#cut-5 name=troikagroup}
